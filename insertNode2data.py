...
while itime <= 1152000:
    ...
    session.insert_record("root.lz4_node_02.percent", itime, measurements_, data_types_, cpuPercent)
    
    cpuFreq = psutil.cpu_freq(percpu=True)
    pcf = [freq[0] for freq in cpuFreq]
    session.insert_record("root.lz4_node_02.freq", itime, measurements_, data_types_, pcf)
    itime += 1

# execute sql query statement
itime = itime-10
with session.execute_query_statement(
    "select * from root.lz4_node_02.freq"
) as session_data_set:
